...
{
    QFontMetricsF fm(this->font()); //用于计算字符串所占空间的类
    m_lineHeight = fm.lineSpacing();
    int nCount = src.count("\n");
    int nMaxWidth = 0;
    int nletter = 0;
    if(nCount == 0) {//开始输出聊天信息
        for(int i=0;i<src.length();i++){
            if((src.at(i)>='a'&&src.at(i)<='z')||(src.at(i)>='A'&&src.at(i)<='Z')){
                nletter+=1;
            }
        }
        nMaxWidth = fm.width(src)+src.length();
        QString value = src;
        ...
        for(int i = 0; i < (nCount + 1); i++) {
            ...
            for(int j=0;j<value.length();j++){
                if((value.at(j)>='a'&&value.at(j)<='z')||(value.at(j)>='A'&&value.at(j)<='Z')){
                    nletter++;
                }
            }
            nMaxWidth = fm.width(value) > nMaxWidth ? fm.width(value)+value.length()/2 : nMaxWidth;
